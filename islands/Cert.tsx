  ...
  return (
    <div>
      <form class = "flex flex-col space-y-4 items-center" onSubmit={(e) => getValue(e)}>
        ...
        />
        <button type="submit" class = "px-4 py-2 text-center text-white rounded-xl bg-purple-700 hover:bg-purple-500 transition duration-500 ease-in-out transform hover:scale-110">Search</button>
      </form>
...
function GetCertificate({ id }: { id: string }) {
  ...
  fetch(`/get/${id}`).then((res) => res.text()).then((res) => setCert(res));
